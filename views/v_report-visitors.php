<div class="page-header">
    <h2><i class="fa fa-signal"></i> Links Report</h2>
    <p>Tracking link overview <?php echo $date_selection['display_title']; ?>. </p>
</div>

<div class="btn-toolbar">
    <div class="btn-group pull-left">
        <a href="?page=sh_reports_page" class="btn btn-default">Overview</a>
        <a href="?page=sh_reports_page&action=report-referrers" class="btn btn-default">Referrers</a>
        <a href="?page=sh_reports_page&action=report-countries" class="btn btn-default">Countries</a>
        <a href="?page=sh_reports_page&action=report-links" class="btn btn-default">Links</a>
        <a href="?page=sh_reports_page&action=report-source" class="btn btn-default">Campaigns</a>
        <a href="?page=sh_reports_page&action=report-visitors" class="btn btn-primary active">Visitors</a>
    </div>
    <div class="btn-group pull-right">
        <?php echo $date_selection['widget']; ?>
    </div>
</div>

<p>&nbsp;</p>

<table id="report_visitors" class="table table-striped">
    <thead>
        <tr>
                <th>CTID</th>
                <th>IP Address</th>
                <th>Country</th>
                <th>City</th>
                <th>Referrer</th>
                <th>Tracker</th>
                <th>Destination</th>
                <th>Campaign</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td colspan="8">Loading</td>
        </tr>
    </tbody>
    <tfoot>
        <tr>
            <th><strong>Total</strong></th>
            <th>&nbsp;</th>
            <th>&nbsp;</th>
            <th>&nbsp;</th>
            <th>&nbsp;</th>
            <th>&nbsp;</th>
            <th>&nbsp;</th>
            <th>&nbsp;</th>
        </tr>
    </tfoot>
</table>
</div>

