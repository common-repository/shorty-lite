...
//            $site_url = isset($site_url['host']) ? $site_url['host'] : $_SERVER['HTTP_HOST'];
        }
    ...
    /**
     * check if tracking link is exist
     * @global type $wpdb
     * @param type $field
     * @param type $input
     * @param type $param
     * @return type
     */
    protected function validate_tracking_link_exist($field, $input, $param = null) {
        global $wpdb;
        /**
         * remove domain
         */
        $tracking_link = $input[$field];
        $tracking_link = str_replace($this->current_domain(TRUE, FALSE, TRUE), '', $tracking_link);
        
        /**
         * check if tracking link is exist
         */
        $table = $wpdb->prefix . SH_PREFIX . 'links';
        $link = $wpdb->get_row($wpdb->prepare("SELECT COUNT(id) FROM {$table} WHERE tracking_link = %s", $tracking_link), ARRAY_N);

        /**
         * for the case shorty domain is same as wordpress, we check for the existing post slug
         */
        $post = array();
        $post[0] = 0;
        if ($this->current_domain(TRUE, FALSE, TRUE) != $this->current_domain(TRUE, TRUE, TRUE)) {
            $table = $wpdb->prefix . 'posts';
            $post = $wpdb->get_row($wpdb->prepare("SELECT COUNT(id) FROM {$table} WHERE post_name != %s", $tracking_link), ARRAY_N);
        }
...
//        print_r($sql[0]);
     ...
     * 
     */
    protected function validate_post_url($field, $input, $param = null) {
        global $wpdb;

        $post_url = $input[$field];
        $post_id = url_to_postid($post_url);
        $table = $wpdb->prefix . 'posts';
        $sql = $wpdb->get_row($wpdb->prepare("SELECT COUNT(ID) FROM {$table} WHERE ID = %d", $post_id), ARRAY_N);
        if ($sql[0] == 0) {
            return array(
                'field' => $field,
                'value' => $input[$field],
    ...
    protected function validate_currency($field, $input, $param = null) {
        ...
        if (!preg_match("/^\d+(?:\.\d{2})?$/", $input[$field]) !== false) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
    ...
    /**
     * Check tracking link for the uniqueness
     */
    protected function validate_tracking_link($field, $input, $param = null) {
        global $wpdb;
        $tracking_link = $input[$field];
        ...
        if ($param == null) {
            $result = $wpdb->get_row($wpdb->prepare("SELECT COUNT(id) FROM {$table} WHERE tracking_link = %s", $tracking_link), ARRAY_N);
        } else {
            $result = $wpdb->get_row($wpdb->prepare("SELECT COUNT(id) FROM {$table} WHERE tracking_link = %s AND id != %d", $tracking_link, (int) $param), ARRAY_N);
        }
         ...
         */
        $table = $wpdb->prefix . 'posts';
        $post_result = $wpdb->get_row($wpdb->prepare("SELECT COUNT(id) FROM {$table} WHERE post_status !='trash' AND post_name = %s", $tracking_link), ARRAY_N);
        if (($result[0] > 0) || ($post_result[0] > 0)) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
    ...
    protected function validate_split_test_tracking_link($field, $input, $param = null) {
        global $wpdb;
        $tracking_link = $input[$field];

        $table = $wpdb->prefix . SH_PREFIX . 'split_tests';
    ...
    protected function validate_alpha_dash_slash($field, $input, $param = null) {
        if (!isset($input[$field]) || empty($input[$field])) {
            return;
        }
...
// Let's fetch explicit field names if they exist
            ...
            switch ($e['rule']) {
                case 'mismatch' :
                    $resp[$field_key] = "There is no validation rule for $field";
                    break;
                case 'validate_required':
                    $resp[$field_key] = "The $field field is required";
                    break;
                case 'validate_valid_email':
                    $resp[$field_key] = "The $field field is required to be a valid email address";
                    break;
                case 'validate_max_len':
                    $resp[$field_key] = "The $field field needs to be $param or shorter in length";
                    break;
                case 'validate_min_len':
                    $resp[$field_key] = "The $field field needs to be $param or longer in length";
                    break;
                case 'validate_exact_len':
                    $resp[$field_key] = "The $field field needs to be exactly $param characters in length";
                    break;
                case 'validate_alpha':
                    $resp[$field_key] = "The $field field may only contain alpha characters(a-z)";
                    break;
                case 'validate_alpha_numeric':
                    $resp[$field_key] = "The $field field may only contain alpha-numeric characters";
                    break;
                case 'validate_alpha_dash':
                    $resp[$field_key] = "The $field field may only contain alpha characters &amp; dashes";
                    break;
                case 'validate_numeric':
                    $resp[$field_key] = "The $field field may only contain numeric characters";
                    break;
                case 'validate_integer':
                    $resp[$field_key] = "The $field field may only contain a numeric value";
                    break;
                case 'validate_boolean':
                    $resp[$field_key] = "The $field field may only contain a true or false value";
                    break;
                case 'validate_float':
                    $resp[$field_key] = "The $field field may only contain a float value";
                    break;
                case 'validate_valid_url':
                    $resp[$field_key] = "The $field field is required to be a valid URL";
                    break;
                case 'validate_url_exists':
                    $resp[$field_key] = "The $field URL does not exist";
                    break;
                case 'validate_valid_ip':
                    $resp[$field_key] = "The $field field needs to contain a valid IP address";
                    break;
                case 'validate_valid_cc':
                    $resp[$field_key] = "The $field field needs to contain a valid credit card number";
                    break;
                case 'validate_valid_name':
                    $resp[$field_key] = "The $field field needs to contain a valid human name";
                    break;
                case 'validate_contains':
                    $resp[$field_key] = "The $field field needs to contain one of these values: " . implode(', ', $param);
                    break;
                case 'validate_street_address':
                    $resp[$field_key] = "The $field field needs to be a valid street address";
                    break;
                case 'validate_date':
                    $resp[$field_key] = "The $field field needs to be a valid date";
                    break;
                case 'validate_min_numeric':
                    $resp[$field_key] = "The $field field needs to be a numeric value, equal to, or higher than $param";
                    break;
                case 'validate_max_numeric':
                    $resp[$field_key] = "The $field field needs to be a numeric value, equal to, or lower than $param";
                    break;
                case 'validate_min_age':
                    $resp[$field_key] = "The $field field needs to have an age greater than or equal to $param";
                    break;
                case 'validate_currency':
                    $resp[$field_key] = "The $field field needs to be a valid currency format eg: 1.97";
                    break;
                case 'validate_tracking_link':
                    $resp[$field_key] = "The $field already exist or used in post";
                    break;
                case 'validate_split_test_tracking_link':
                    $resp[$field_key] = "The $field already exist or used in post";
                    break;
                case 'validate_tracking_link_exist':
                    $resp[$field_key] = "The $field is not exist or conflict with post slug";
                    break;
                case 'validate_total_percentage':
                    $resp[$field_key] = "Total % should be 100%";
                    break;
//                case 'validate_tracking_link_edit':
//                    $resp[$field_key] = "The $field already exist";
//                    break;
                default:
                    $resp[$field_key] = "The $field field is invalid";
            }
        }
    ...
    /**
     * To validate total percentage is 100%
     * 
     */
    protected function validate_total_percentage($field, $input, $param = null) {
        ...
        if (filter_var($input[$field], FILTER_VALIDATE_INT) !== false) {
            ...
            list($field_name, $total_field) = explode(';', $param);
            $total = 0;
            for ($i = 1; $i <= $total_field; $i++) {
                $total+=isset($_POST[$field_name . $i]) ? $_POST[$field_name . $i] : 0;
            }
            if ($total != 100) {
                return array(
                    'field' => $field,
...
// Let's fetch explicit field names if they exist
            ...
            switch ($e['rule']) {
                ...
                case 'validate_min_age':
                    ...
                    break;
                case 'validate_currency':
                    $resp[] = "The $field field needs to be a valid currency format eg: 1.97";
                    break;
                case 'validate_tracking_link':
                    $resp[] = "The tracking link already exist";
                    break;
                case 'validate_tracking_link_edit':
                    $resp[] = "The $field already exist";
                    break;
                default:
...
// Let's fetch explicit field names if they exist
            ...
            switch ($e['rule']) {
                ...
                case 'validate_min_age':
                    ...
                    break;
                case 'validate_currency':
                    $resp[$field] = "The $field field needs to be a valid currency format eg: 1.97";
                    break;
                case 'validate_tracking_link':
                    $resp[$field] = "The $field already exist";
                    break;
                case 'validate_tracking_link_edit':
                    $resp[$field] = "The $field already exist";
                    break;
